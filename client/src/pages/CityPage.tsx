...
export default function CityPage() {
  ...
  const label = city
    ...
    .join(" ");
  const { data = [], isLoading } = trpc.profiles.list.useQuery({ city: label });
  return (
    <div className="min-h-screen bg-[#222222] text-white">
      <Seo
        title={`Acompanhantes em ${label} — Só Models`}
        description={`Encontre acompanhantes por cidade, categoria e disponibilidade em ${label}.`}
        path={`/cidade/${city}`}
      />
      ...
      <main className="container py-12">
        ...
        <h1 className="font-display text-5xl">Acompanhantes em {label}</h1>
        <p className="mt-4 max-w-2xl text-white/55">
          Explore perfis publicados, informações públicas, preferências e
          disponibilidade em {label}.
        </p>
        {!isLoading && !data.length ? (
          <div className="empty-state mt-10">
            ...
            <h3>Nenhum perfil encontrado em {label}</h3>
            <p>
              Quando houver perfis aprovados nesta cidade, eles aparecerão aqui.
            </p>
            <Link href="/">
              <Button className="mt-5 bg-[#ff4764] text-[#222222]">
                ...
                <Card className="profile-card">
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      {profile.categories.slice(0, 3).map((tag: string) => (
                        <Badge
                          key={tag}
                          variant="outline"
